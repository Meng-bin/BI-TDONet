...
def IDP_BIE_cupy(x, y, dx, dy, ddx, ddy, f, method="solve", tol=1e-15, restart=10):
    ...
    I = cp.eye(x.shape[1])
    if method == "solve":
        phi = cp.linalg.solve(I - K.T * 2 * cp.pi / x.shape[1], cp.reshape(f, [-1, 1]))
    elif method == "gmres":
        phi, _ = cpgmres(
            I - K.T * 2 * cp.pi / x.shape[1],
            cp.reshape(f, [-1, 1]),
            tol=tol,
            restart=restart,
        )
...
def IDP_BIE(x, y, dx, dy, ddx, ddy, f, method="solve", tol=1e-15, restart=10):
    ...
    I = np.eye(x.shape[1])
    if method == "solve":
        phi = np.linalg.solve(I - K.T * 2 * np.pi / x.shape[1], np.reshape(f, [-1, 1]))
    elif method == "gmres":
        phi, _ = npgmres(
            I - K.T * 2 * np.pi / x.shape[1],
            np.reshape(f, [-1, 1]),
            tol=tol,
            restart=restart,
        )
